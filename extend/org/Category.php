...
class Category {
    ...
    private $fields_string="";													//穿入的数组转为字符串                                  
    ...
    public function __construct($model = '', $fields = array()) {
        ...
        if (!empty($fields)){
        	unset($fields['3']);
        	$this->fields_string=implode(",", $fields);
        }else {
        	$this->fields_string="*";
        }
    }
    ...
    private function _findAllCat($condition, $orderby = NULL) { 
        $this->rawList = empty($orderby) ? $this->model->where($condition)->field($this->fields_string)->select() : $this->model->where($condition)->field($this->fields_string)->order($orderby)->select(); 
    }
    ...
    public function getList($condition = NULL, $cid = 0, $orderby = NULL) {
        ...
        $this->_findAllCat($condition, $orderby, $orderby);//获取所有数据
    ...
    public function getTree($data, $cid = 0) {
        $this->rawList=$this->formatList=array();
        $this->rawList = $data;
        $this->_searchList($cid);  
        return $this->formatList;
    }
